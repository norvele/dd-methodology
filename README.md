...
В этом документе будет описана методология, ее понятия, синтаксис, принципы, рекомендации, примеры. Методология позволяет полностью исключить пересечения\протечки стилей, построить удобную структуру проекта, в которой легко находить нужное, легко и безопасно изменять код.

## Основные понятия

Методология идеологически\религиозно относится к подвидам БЭМ, поэтому у нее есть похожие вещи: компонент, элемент, модификатор. Но есть и много слишком много отличий, о которых пойдет речь далее. Вот основные понятия, которые будут расскрыты далее:

* Контекст
* Элемент
* Компонент
* Модификатор
* Абстрактный и уточняющий компоненты
...
Как можно было догодаться - у элемента всегда есть контекст, но не всегда это контекст своего компонента. Далее я расскажу, что это за непонятные элементы и компоненты. 
...
Из примеров можно заметить, что элементы могут быть вложенными, а также могут находиться в контексте некоего root элемента (например: шапка). Такие root элементы именуются компонентами и о них речь пойдет ниже.
...
Эти свойства могут быть применены к элементу только его контекстом. Если просто - только родитель может влиять на расположение элемента внутри себя, а также родитель вправе устанавливать окончательный вид элемента.
...
Важно понимать, что модификаторы должны влиять только на внешний вид, не затрагивая разметку. Т.е. если установка элементу модификатора подразумевает еще и изменение html кода этого элемента, то это значит, что тут должны использоваться абстрактные и уточняющие компоненты.
...
Все имена селекторов разбиты на логические части, отделяемые дефисом. В случае с селектором элемента они разделяют между собой элементы\компоненты. В случае с модификатором они образуют неймспейсы, которые позволяют проще понимать на что они влияют. Например: помимо изменения размера самой кнопки мы можем захотеть изменять размер внутренних отступов кнопки, тогда запись `_extraLarge` будет уже не такой прозрачной, а вот записи `_size-extraLarge` и `_gutters-extraLarge` вполне конкретно сообщают нам что мы будем модифицировать.
...
Описание компонента состоит из 4 блоков, которые должны идти в представленном выше порядке. Порядок очень важен, почему так - будет описано далее.
