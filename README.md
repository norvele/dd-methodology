...
Методология идеологически\религиозно относится к подвидам БЭМ, поэтому у нее есть похожие вещи: компонент, элемент, модификатор. Но есть и много даже слишком много отличий, о которых пойдет речь далее.
...
Эти свойства могут быть применены к элементу только его контекстом. Если просто - только родитель может влиять на расположение элемента внутри себя. Также родитель вправе устанавливать окончательный вид элемента.
